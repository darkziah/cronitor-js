...
function Cronitor(apiKey, config={}) {
    ...
    const headers = {
        ...
        'Authorization': 'Basic ' + new Buffer.alloc(5, apiKey + ':').toString('base64')
    };
    ...
    this._api = {
        ...
        pingUrl: (key) => `http://localhost:3000/api/heartbeat/${apiKey}/${key}`,//`https://cronitor.link/ping/${apiKey}/${key}`,
        monitorUrl: (key) => `http://localhost:3000/api/job/${apiKey}/${key}`,
        axios: axios.create({
            baseURL: '',
            timeout: 5000,
            headers,
